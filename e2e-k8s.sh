...
create_cluster() {
  ...
  KUBE_VERSION="$(docker run --rm --entrypoint=cat "kindest/node:v1.20.2" /kind/version)"
...
main() {
  ...
  create_cluster || res=$?
  # run_tests || res=$?
  # helm install --set standalone.enabled=true --set image.all.repository=registry.zilliz.com/milvus-distributed/milvus-distributed --set image.all.tag=r0.3-test-release --set image.all.pullPolicy=Always --set logsPersistence.persistentVolumeClaim.accessModes=ReadWriteOnce --set logsPersistence.enabled=true --set logsPersistence.mountPath=/milvus-distributed/logs test milvus-helm-charts/charts/milvus-ha/ 
  cleanup || res=$?
  exit $res
}
